...
serve(async (req) => {
  ...
  try {
    ...
    if (!RESEND_API_KEY) {
      console.error("ERRO: RESEND_API_KEY não configurada no ambiente Supabase")
    ...
    const sendEmail = async (to: string[], subject: string, html: string) => {
      ...
      console.log("Enviando e-mail:", JSON.stringify(payload))
      ...
      if (!response.ok) {
        console.error("Erro ao enviar email:", text)
        throw new Error(`Falha ao enviar e-mail: ${text}`)
      }

      console.log("E-mail enviado com sucesso:", text)
    }

    // Email para a equipe Pontedra (notificação)
    const emailEquipeHTML = `
      <!DOCTYPE html>
      <html>
        <head>
          <meta charset="utf-8">
                    ...
                    <td style="padding: 30px;">
                      <table width="100%" cellpadding="8" cellspacing="0">
                        <tr>
                          ...
                          <td style="color: #212529; font-size: 14px; padding: 8px 0;">${leadData.nome || "Não informado"}</td>
                        </tr>
    ...
    await Promise.all([
      sendEmail(internalRecipients, `🔔 Novo Lead: ${leadData.nome} - ${origemTexto}`, emailEquipeHTML),
      sendEmail([leadData.email], "✅ Recebemos sua mensagem!", emailClienteHTML),
    ])
    ...
    console.error('Erro geral na função send-lead-emails:', error)
