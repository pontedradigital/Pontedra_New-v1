...
import ReportDownloadDialog from '@/components/dashboard/master/reports/ReportDownloadDialog';

export default function ReportsPage() {
  const { profile, loading: authLoading } = useAuth();
  const [selectedDate, setSelectedDate] = useState(new Date());
  const [selectedChartType, setSelectedChartType] = useState<'line' | 'bar'>('line');
  const [isDownloadDialogOpen, setIsDownloadDialogOpen] = useState(false); // Novo estado para o diálogo de download
    ...
    <DashboardLayout>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <Button onClick={() => setIsDownloadDialogOpen(true)} className="bg-secondary hover:bg-secondary/80 text-secondary-foreground rounded-md">
